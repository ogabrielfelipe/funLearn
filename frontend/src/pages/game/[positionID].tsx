...
export default function GameStudent() {
  ...
  const [descriptionAnswerCorrect, setDescriptionAnswerCorrect] =
    useState<string>("");
  ...
  async function findDetailsAsk(
    ...
    setLoading(true);
    await apiClient
      .get(`/game/find/ask/${askID}/${gameID}`)
      .then((resp) => {
        if (resp.status === 200) {
          ...
          setAskLevel(
            resp.data.level === "INITIAL"
              ? "Iniciante"
              : resp.data.level === "ADVANCED"
              ? "Avançada"
              : "Intermediária"
          );
  ...
  async function findAsksByPositionID(
    positionID: string,
    restartTime: boolean
  ) {
    setLoading(true);
    apiClient
      .get(`/game/find/askByPosition/${positionID}`)
      .then(async (resp) => {
  ...
  async function getPosition(
    ...
    setLoading(true);
    apiClient
      .get(`/game/find/position/${positionID}`)
      .then(async (resp) => {
  ...
  function handleShowModelTimeOut() {
    const modelOpen = document.getElementById("modelTimeOut");
      ...
      .then((resp) => {
        ...
        if (resp.data.finishedGame.id) {
          ...
          if (resp.data.isCorrect) {
            ...
            setCountTipUsed(0);
            setDescriptionAnswerCorrect(
              resp.data.changeGameResult.ask.answer[0].description
            );
  ...
  function countDownTimeAsk() {
    ...
    const countDownTime = setInterval(() => {
      ...
      const timeRemainingFormatted = new Intl.DateTimeFormat("pt-br", {
        dateStyle: "short",
        timeStyle: "medium",
      }).format(timeRemaining);
      const timeNowFormatted = new Intl.DateTimeFormat("pt-br", {
        dateStyle: "short",
        timeStyle: "medium",
      }).format(dateNow);
      const diff = moment(timeRemainingFormatted, "DD/MM/YYYY HH:mm:ss").diff(
        moment(timeNowFormatted, "DD/MM/YYYY HH:mm:ss"),
        "seconds"
      );
    ...
    <>
      ...
      <header className={styles.header}>
        <div className={styles.contentGame}>
          ...
          <div className={styles.contentCoin}>
            <Coin />
            <strong className={styles.textCoin}>{coinPosition}</strong>
          </div>
        ...
        <div className={styles.container}>
          <div className={styles.containerAsk}>
            <strong className={styles.strongQuestion}>
              <span>
                Pergunta:{" "}
                <span className={styles.contentDetailAsk}>
                  {" "}
                  {parseInt(numberAsk.split("/")[0]) + 1 > 10
                    ? 10
                    : parseInt(numberAsk.split("/")[0]) + 1}{" "}
                </span>
              </span>
              <span>
                Nível:{" "}
                <span className={styles.contentDetailAsk}>{askLevel}</span>
              </span>
              <span>
                Valor:{" "}
                <span className={styles.contentDetailAsk}>{askGeralPoint}</span>
              </span>
            </strong>
            ...
            <span className={styles.question}>
              ...
              {askQuestion.split("`")[1] && (
                <pre
                  className="prettyprint lang-sql"
          ...
          <div className={styles.containerAnswTip}>
            <div className={styles.contentTipAnswer}>
              <strong
                className={styles.textTimeAndTip}
                style={{ cursor: "pointer" }}
                onClick={handleShowModelTips}
              >
                <Lamp />
                Dica
              </strong>
                ...
                return (
                  <>
                    <div
                      key={index}
                      onClick={() => {
                        selectAnswer(value.id);
                      }}
                      id={value.id}
                      className={styles.contentAnswer}
                    >
                      <span hidden>{value.id}</span>
                      <div className={styles.alphaLetters}>
                        <a href="#" id={`alpha-${value.id}`}>
                          {alpha[index]}
                        </a>
                      </div>
                      <div>
      ...
      <div className={styles.modelTips} id={"modelTips"}>
        ...
        <div className={styles.contentModelTips}>
          ...
          <p className={styles.nameTip}>{tipName}</p>
        </div>
      </div>
        ...
        <div className={styles.contentModelTimeOut}>
          ...
          <div className={styles.textYouWin}>
            ...
            <div className={styles.contentPoint}>
              <span>
                {" "}
                Pergunta: <span style={{ color: "#b18e2e" }}>
                  {" "}
                  {pointAsk}{" "}
                </span>{" "}
                pontos
              ...
              <span>
                {" "}
                Dicas usadas:{" "}
                <span style={{ color: "#b18e2e" }}> {tipUsedAnswered} </span>
              </span>
              <span>
                {" "}
                Você recebeu:{" "}
                <span className={styles.point}> {pointAnswer} </span> pontos.
              </span>
            </div>
          </div>
      ...
      <div className={styles.modelTimeOut} id={"modelWrong"}>
        <div className={styles.contentModelTimeOut}>
          ...
          <div className={styles.textYouWin}>
            <span style={{ color: "#FF1B1B", fontSize: "2.5rem" }}>
              Você Errou!!!
            </span>
            <br />
            <br />
            <span>
              ...
              <span style={{ color: "#FCC123" }}>
                {descriptionAnswerCorrect}
              </span>
            </span>
          </div>
          ...
          <div className={styles.contentBtn}>
            <ButtonStudenTertiary
              onClick={() => {
                handleFindNewAsk();
                handleCloseModelAnswerIncorrect();
              }}
            >
              Próxima
            </ButtonStudenTertiary>
